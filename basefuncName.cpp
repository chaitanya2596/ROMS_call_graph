...
struct FunctionData
{
    ...
    bool hasMyRank = false; // Flag to indicate presence of MyRank
};
...
void parseFortranFiles(const std::string &directory, std::unordered_map<std::string, FunctionData> &callGraph, bool includeNf90)
{
    ...
    std::regex myRankRegex(R"(\bMyRank\b)"); // Detects MyRank
    ...
    for (const auto &entry : fs::recursive_directory_iterator(directory))
    {
        if (entry.path().extension() == ".f90" || entry.path().extension() == ".F")
        {
            ...
            while (std::getline(file, line))
            {
                ...
                if (std::regex_search(line, myRankRegex))
                {
                    if (!currentFunction.empty())
                    {
                        callGraph[currentFunction].hasMyRank = true;
                    }
                }
            }
        }
